...
description: STRIDE threat model for the Mural skill organized by assets, adversaries, and trust buckets (Browser→Loopback, CLI→Mural, on-disk cache, MCP stdio) with in-code mitigations and acknowledged enterprise readiness gaps
...
| A4 | Cached widget content from Mural            | Tool-call lifetime | MCP tool results may include sticky-note text, attachments, or comments authored by other Mural users. May contain PII or confidential workshop content; downstream agents must treat as untrusted input. |
...
| ADV-f | Hostile or untrusted host process for the MCP stdio server            | MCP transport is process-local stdio; the server trusts the parent process. JSON-RPC inputs are schema-validated; destructive tools require granted scope and an elicitation hook before execution. Mural-authored text in tool results is treated as untrusted by downstream agents. |
...
**Known limitation: plaintext at-rest.** Both the token store and the credential file are mode-0600 plaintext on disk. Stdlib symmetric encryption with a passphrase prompt was rejected because it would prompt on every CLI and MCP invocation (defeating the UX goal) and storing the passphrase next to the ciphertext provides no real defense over POSIX permissions. The credential file shares the same threat model as the token store; users who require stronger at-rest protection wrap invocations with an out-of-band secrets manager (`dotenvx`, `sops exec-env`, `pass`) as documented in [SKILL.md](SKILL.md).

## Bucket B4: MCP stdio

The skill exposes the same operations as the CLI through a hand-rolled stdio MCP server speaking JSON-RPC 2.0 over newline-delimited JSON frames. Protocol versions `2025-11-25` (preferred) and `2025-06-18` (fallback) are negotiated during `initialize`.

### Spoofing

* The transport is process-local stdio. The MCP server trusts that the parent process launched it; there is no network attach surface.
* The server advertises a fixed identity (`name="mural"`, `version="1.0.0"`) and a static capability set (`tools.listChanged=false`); there is no client-side capability advertisement that the host can use to alter dispatch behavior beyond the documented `elicitation` capability.

### Tampering

* Inbound frames are validated against `MURAL_MAX_FRAME_BYTES` (default 4 MiB); oversized frames are rejected before parsing rather than buffered to exhaustion. The reader drains the rest of the offending line so the next frame starts cleanly.
* Tool inputs are validated against each tool's JSON Schema before the handler runs (`_validate_tool_input_schema`). All tool input schemas declare `additionalProperties: false`, so unknown fields are rejected rather than silently ignored.
* The tool registry itself is validated at module load (`_validate_tool_registry`), which guarantees every advertised tool has the required schema and metadata fields before the dispatch loop begins.

### Repudiation

* Tool dispatch logs the tool name, request id, and outcome to stderr so the host can correlate MCP requests with Mural API calls.
* The in-process idempotency cache (`_IDEMPOTENCY_CACHE`, bounded LRU of 128 entries) returns the original result for duplicate `(tool_name, idempotency_key)` pairs within a process lifetime, so repeated create calls cannot be silently double-attributed. The cache is process-local and not persisted across restarts.
...
* Per-tool execution is bounded by `MURAL_TOOL_TIMEOUT_SECS` (default 60s); a slow Mural endpoint cannot wedge the MCP loop. Timeouts are surfaced to the client as JSON-RPC error code `-32000` with `data.tool` and `data.timeout_secs`.
* Frame reads honor `MURAL_MAX_FRAME_BYTES` so a malformed or hostile frame cannot consume unbounded memory.
...
* Tool calls are dispatched on a single-worker `ThreadPoolExecutor` (FIFO, `max_workers=1`); a flood of concurrent calls is serialized rather than spawning unbounded workers.

### Elevation of Privilege

* Every MCP tool dispatches through the same scope-checked code path as the CLI handler (`_require_scope`); there is no MCP-only bypass that skips the granted-scope check before calling a write endpoint.
* Destructive tools require a positive return from `_maybe_elicit` before execution. The elicitation hook **fails closed**: if no hook is registered, or the client did not advertise the `elicitation` capability during `initialize`, or the registered hook raises, the destructive call is refused.
* Destructive tools accept a `dry_run` argument that returns a structured preview without invoking the underlying Mural API call.
* JSON-RPC error envelopes use the standard codes `-32700` (parse), `-32600` (invalid request), `-32601` (method not found), `-32602` (invalid params), `-32603` (internal), and `-32000` (tool timeout). Notification messages (no `id`) never receive a response, matching the JSON-RPC 2.0 contract.
...
| G-REP-1 | No structured or tamper-evident audit log is emitted. Tool dispatch goes to stderr only; there is no signed or append-only sink.                                                                                                                                                                                                                                                                                                                                                                        | Repudiation-Med | Out of scope for the skill; integrate with host telemetry if required.                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| G-INF-4 | Mural payloads returned through MCP tool results may include PII or confidential workshop content (A4). The skill does not classify or redact this content; downstream handling is the host's responsibility.                                                                                                                                                                                                                                                                                           | InfoDisc-Med    | By design; documented in B4 Information Disclosure.                                                                                                                                                                                                                                                                                                                                                                                                                                      |
