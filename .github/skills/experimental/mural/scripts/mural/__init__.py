...
live in this same module; the ``mcp`` subparser is currently a stub that exits
non-zero.
...
def _require_scope(
    ...
    *,
    ...
    other multi-scope tools declare requirements via the ``required_scopes``
    spec field on their :data:`_TOOL_REGISTRY` entry.
    """
    if store is None:
...
def _layout_cli_arguments(args: argparse.Namespace) -> dict[str, Any]:
    """Build the MCP-style ``arguments`` dict from a layout CLI namespace."""
...
# ``OrderedDict``; holds previously formatted MCP tool results keyed by
...
# Client capability snapshot recorded during MCP ``initialize``. Used by
# :func:`_maybe_elicit` and any future bidirectional features.
_CLIENT_CAPABILITIES: dict[str, Any] = {}

# Optional injection point for tests: when set, called as
# ``hook(tool_name, arguments) -> bool``; ``False`` aborts the destructive
# call. The synchronous request/response correlation for the MCP
# ``elicitation/create`` JSON-RPC method is deferred (Phase 4 follow-on);
# until then the framework is exposed so a stub can be plugged in.
_ELICITATION_HOOK: Callable[[str, dict[str, Any]], bool] | None = None


def _maybe_elicit(name: str, arguments: dict[str, Any]) -> bool:
    """Return ``True`` to proceed with a destructive tool call, ``False`` to
    abort. Fails closed when no hook is registered, the client did not
    advertise the ``elicitation`` capability, or the registered hook raises;
    destructive Mural tools must never silently auto-approve."""
    hook = _ELICITATION_HOOK
    if hook is None:
        return False
    if "elicitation" not in (_CLIENT_CAPABILITIES or {}):
        return False
    try:
        return bool(hook(name, arguments))
    except Exception as exc:  # noqa: BLE001
        LOGGER.error(
...
_TOOL_REGISTRY: dict[str, dict[str, Any]] = {
    "mural_workspace_list": {
        ...
        "description": (
            "List Mural workspaces visible to the authenticated user.\n\n"
            "Returns workspaces accessible to the OAuth identity. Supports "
            "pagination via `limit` and `page_size`. Read-only."
        ),
        "input_schema": {
            "type": "object",
    ...
    "mural_workspace_get": {
        ...
        "description": (
            ...
            "When `workspace` is omitted, defaults to the value of the "
            "`MURAL_DEFAULT_WORKSPACE` environment variable. Read-only."
        ),
        "input_schema": {
            "type": "object",
    ...
    "mural_room_list": {
        "title": "List rooms",
        "description": (
            "List rooms within a workspace.\n\n"
            "When `workspace` is omitted, defaults to `MURAL_DEFAULT_WORKSPACE`. "
            "Supports pagination via `limit` and `page_size`. Read-only."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    "mural_room_get": {
        ...
        "description": (
            ...
            "Returns the room metadata. The `room` argument is required and "
            "must be a non-empty string. Read-only."
        ),
        "input_schema": {
            "type": "object",
    ...
    "mural_room_create": {
        ...
        "description": (
            ...
            "Calls `POST /rooms` with `workspaceId` in the request body. "
            "When `workspace` is omitted, defaults to `MURAL_DEFAULT_WORKSPACE`. "
            "`type` defaults to `private`. Requires the `rooms:write` scope."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "type": {"type": "string", "enum": ["private", "open"]},
        ...
        "handler": _tool_room_create,
        "destructive": True,
        "creates": True,
        "required_scopes": ["rooms:write"],
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_mural_list": {
        "title": "List murals",
        "description": (
            "List murals within a workspace.\n\n"
            "When `workspace` is omitted, defaults to `MURAL_DEFAULT_WORKSPACE`. "
            "Supports pagination via `limit` and `page_size`. Read-only."
        ),
        "input_schema": {
    ...
    "mural_mural_create": {
        "title": "Create mural",
        "description": (
            "Create a mural in a room.\n\n"
            "Calls `POST /murals` with `roomId` (integer) and `title` in the "
            "request body. Requires the `murals:write` scope."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "room": {"type": ["string", "integer"], "minLength": 1},
        ...
        "handler": _tool_mural_create,
        "destructive": True,
        "creates": True,
        "required_scopes": ["murals:write"],
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_mural_get": {
        "title": "Get mural",
        "description": (
            "Get a single mural by id.\n\n"
            "The `mural` argument uses Mural's `workspace.id` notation. "
            "Returns mural metadata only (not widgets). Read-only."
        ),
        "input_schema": {
            "type": "object",
    ...
    "mural_widget_list": {
        "title": "List widgets",
        "description": (
            "List widgets on a mural with optional type and parent filters.\n\n"
            "Filter by widget `type` (e.g. `sticky_note`, `text_box`) or by "
            "`parent_id` to scope to children. Supports pagination via "
            "`limit` and `page_size`. Read-only."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    "mural_widget_get": {
        ...
        "description": (
            ...
            "Returns the widget object including geometry, style, and type. "
            "Both `mural` and `widget` are required. Read-only."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    "mural_widget_update": {
        "title": "Update widget",
        "description": (
            "PATCH a widget with a JSON body.\n\n"
            "Mutates the widget identified by `mural` and `widget` using the "
            "fields provided in `body`. Set `dry_run: true` to validate "
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "widget": {"type": "string", "minLength": 1},
                "body": {"type": "object"},
                "require_author_tag": _REQUIRE_AUTHOR_TAG_PROPERTY,
                "force_human": _FORCE_HUMAN_PROPERTY,
                "dry_run": _DRY_RUN_PROPERTY,
            },
    ...
    "mural_widget_delete": {
        ...
        "description": (
            ...
            "Returns `{ok: true, deleted: <id>}` on success. Set "
            "`dry_run: true` to preview without deleting. Destructive and "
            "idempotent."
        ),
        "input_schema": {
            "type": "object",
    ...
    "mural_widget_create_sticky_note": {
        ...
        "description": (
            "Create a sticky-note widget on a mural.\n\n"
            "Required fields are `mural`, `text`, `x`, and `y`. Optional "
            "`shape`, `width`, `height`, and `style` customize appearance. "
            "Use `idempotency_key` to safely retry; `dry_run` to preview."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    "mural_widget_create_textbox": {
        ...
        "description": (
            "Create a textbox widget on a mural.\n\n"
            "Required fields are `mural`, `text`, `x`, and `y`. Optional "
            "`width`, `height`, and `style` customize appearance. Use "
            "`idempotency_key` to safely retry; `dry_run` to preview."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    "mural_widget_create_shape": {
        ...
        "description": (
            ...
            "Required fields are `mural`, `shape`, `x`, and `y`. Optional "
            "`width`, `height`, `text`, and `style` customize the shape. "
            "Use `idempotency_key` to safely retry; `dry_run` to preview."
        ),
        "input_schema": {
            "type": "object",
    ...
    "mural_widget_create_arrow": {
        ...
        "description": (
            "Create an arrow widget connecting two points on a mural.\n\n"
            "Required fields are `mural`, `x1`/`y1` (start) and `x2`/`y2` "
            "(end). Optional `style` customizes appearance. Use "
            "`idempotency_key` to safely retry; `dry_run` to preview."
        ),
        "input_schema": {
            "type": "object",
    ...
    "mural_widget_create_image": {
        ...
        "description": (
            "Create an image widget on a mural from a local file.\n\n"
            "Uploads the file via Azure Blob SAS and attaches it as a widget. "
            "`alt_text` is required for accessibility (WCAG 2.2 SC 1.1.1). "
            "Use `idempotency_key` to safely retry; `dry_run` to preview."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "alt_text": {
                    ...
                    "minLength": 1,
                    "description": (
                        "Alternative text describing the image. Required for "
                        "accessibility (WCAG 2.2 SC 1.1.1)."
                    ),
                },
                "x": _WIDGET_XY_PROPERTY,
                "y": _WIDGET_XY_PROPERTY,
            ...
            "required": ["mural", "file", "alt_text", "x", "y"],
            "additionalProperties": False,
        },
    ...
    "mural_tag_list": {
        "title": "List tags",
        "description": (
            "List tags defined on a mural.\n\n"
            "Returns the tag manifest for the mural. Use with `mural_tag_apply` "
            "to attach tags to widgets. Read-only."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_tag_list,
        "annotations": {"readOnlyHint": True},
    },
    "mural_tag_create": {
        "title": "Create tag",
        "description": (
            "Create a new tag on a mural.\n\n"
            "Tag `text` is required and capped at 25 characters by Mural. "
            "Optional `color` is an API-defined token. Returns the new tag "
            "record. Use `idempotency_key` to safely retry."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "mural": {"type": "string", "minLength": 1},
                "text": _TAG_TEXT_PROPERTY,
        ...
        "handler": _tool_tag_create,
        "destructive": True,
        "creates": True,
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_tag_apply": {
        "title": "Apply tag to widget",
        "description": (
            "Attach an existing or newly-created tag to a widget.\n\n"
            "Provide `tag` (id) for an existing tag, or `text` to lookup or "
            "create one via the tag manifest helper. The mural cap on tag "
            "count is reported as a validation error if exceeded."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_tag_apply,
        "destructive": True,
        "creates": True,
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_tag_remove": {
        "title": "Remove tag from widget",
        "description": (
            "Detach a tag from a widget.\n\n"
            "Removes the tag-widget association without deleting the tag "
            "definition. Set `dry_run: true` to preview. Idempotent."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "widget": {"type": "string", "minLength": 1},
                "tag": {"type": "string", "minLength": 1},
                "force_reserved": _FORCE_RESERVED_PROPERTY,
                "dry_run": _DRY_RUN_PROPERTY,
            },
            "required": ["mural", "widget", "tag"],
            "additionalProperties": False,
        },
        "handler": _tool_tag_remove,
        "destructive": True,
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_area_list": {
        "title": "List areas",
        "description": (
            "List areas (containers) on a mural.\n\n"
            "Areas group child widgets and define a layout (`free`, `column`, "
            "or `row`). Read-only."
        ),
        "input_schema": {
            "type": "object",
        ...
        "handler": _tool_area_list,
        "annotations": {"readOnlyHint": True},
    },
    "mural_area_get": {
        "title": "Get area",
        "description": (
            "Get a single area by id (cached in-process).\n\n"
            "Populates the per-process `_area_cache` so subsequent traversal "
            "calls avoid re-fetching the same area. Read-only."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_area_get,
        "annotations": {"readOnlyHint": True},
    },
    "mural_area_create": {
        "title": "Create area",
        "description": (
            "Create a new area on a mural.\n\n"
            "Required: `mural`, `title`. Optional: geometry (`x`/`y`/`width`/"
            "`height`), `layout` (`free`/`column`/`row`), and `parent_id` for "
            "nesting. Use `idempotency_key` to safely retry."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "width": {"type": "number"},
                "height": {"type": "number"},
                "layout": {"type": "string", "enum": ["free", "column", "row"]},
                "parent_id": {"type": "string", "minLength": 1},
                "dry_run": _DRY_RUN_PROPERTY,
                "idempotency_key": _IDEMPOTENCY_KEY_PROPERTY,
            },
            "required": ["mural", "title"],
            "additionalProperties": False,
        },
        "handler": _tool_area_create,
        "destructive": True,
        "creates": True,
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    ...
    "mural_widget_get_with_context": {
        ...
        "description": (
            "Get a widget plus its area-chain ancestry and sibling widgets.\n\n"
            "Returns `{widget, area_chain, siblings, cluster}` where "
            "`area_chain` walks parentId pointers (cycle-safe, depth-capped) "
            "and `siblings` excludes the requested widget. `cluster` is "
            "reserved for a future Phase 4 enrichment and is currently null."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_widget_get_with_context,
        "annotations": {"readOnlyHint": True},
    },
    "mural_widget_list_with_context": {
        "title": "List widgets with context",
        "description": (
            "List widgets including each one's area-chain ancestry.\n\n"
            "Filter by `type` and/or `parent_id`. Each returned record is "
            "`{widget, area_chain, cluster}`. Use to navigate parentId-aware "
            "groupings without separate area lookups."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_widget_list_with_context,
        "annotations": {"readOnlyHint": True},
    },
    "mural_widget_create_bulk": {
        "title": "Create widgets in bulk",
        "description": (
            f"Create up to {MAX_BULK_WIDGETS} widgets on a mural via one POST "
            "per widget to the matching per-type endpoint.\n\n"
            "`widgets` is a JSON array of objects, each with a `type` field "
            "(e.g. `sticky_note`, `textbox`, `shape`) plus the body fields "
            "that type accepts; each widget is dispatched to its matching "
            "endpoint (e.g. `/widgets/sticky-note`, `/widgets/textbox`). "
            f"Hard-capped at {MAX_BULK_WIDGETS} per call. Returns "
            "`{succeeded, skipped, failed, warnings}`. When `atomic=True` "
            "the run aborts on the first per-widget failure with "
            "EX_TEMPFAIL=75; otherwise remaining widgets are attempted "
            "and failures are reported in `failed[]`."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
            ...
            "required": ["mural", "widgets"],
            "additionalProperties": False,
        },
        "handler": _tool_widget_create_bulk,
        "destructive": True,
        "creates": True,
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_widget_update_bulk": {
        "title": "Update widgets in bulk",
        "description": (
            "Update many widgets on a mural concurrently with backpressure.\n\n"
            f"`updates` is a JSON array of `{{widget_id, body}}` entries, "
            f"hard-capped at {MAX_BULK_WIDGETS} per call. Returns "
            "`{succeeded, failed, warnings}`. When `atomic=True` the run "
            "aborts on the first per-widget failure with EX_TEMPFAIL=75."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "atomic": {"type": "boolean"},
                "require_author_tag": {"type": "boolean"},
                "force_human": {"type": "boolean"},
            },
            "required": ["mural", "updates"],
            "additionalProperties": False,
        },
        "handler": _tool_widget_update_bulk,
        "destructive": True,
        "required_scopes": ["murals:write"],
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_mural_duplicate": {
        "title": "Duplicate mural",
        "description": (
            "Duplicate a mural and return the new mural id.\n\n"
            "Calls `POST /murals/{mural}/duplicate`. Widget identifiers are "
            "not preserved across duplication; consumers that need a tag map "
            "should call `mural_clone_with_tags` instead."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_mural_duplicate,
        "destructive": True,
        "creates": True,
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_clone_with_tags": {
        "title": "Clone mural with tags",
        "description": (
            "Duplicate a mural and re-create the source mural's tag set on "
            "the new mural.\n\n"
            "Returns `{source_mural_id, new_mural_id, tag_count, tag_map, "
            "warnings}`. Widget ids are not preserved across duplication."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_clone_with_tags,
        "destructive": True,
        "creates": True,
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_template_instantiate": {
        "title": "Instantiate template",
        "description": (
            "Create a new mural from an existing template.\n\n"
            "Calls `POST /templates/{template}/instantiate` with the target "
            "`workspace` and optional `room`. Requires `templates:write` in "
            "addition to `murals:write`."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_template_instantiate,
        "destructive": True,
        "creates": True,
        "required_scopes": ["murals:write", "templates:write"],
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_template_create": {
        "title": "Create template from mural",
        "description": (
            "Create a new template from an existing mural.\n\n"
            "Calls `POST /murals/{mural}/template` with the target "
            "`workspace` and optional `room`/`name`. Requires "
            "`templates:write` in addition to `murals:write`."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    "mural_template_list": {
        "title": "List known templates",
        "description": (
            "List templates from the local skill registry.\n\n"
            "Returns `{templates: [{id, label}, ...]}`. The Mural API does "
            "not expose a `GET /templates` endpoint, so this verb reads from "
            "a static registry maintained inside the skill. Read-only; no "
            "scopes required."
        ),
        "input_schema": {
            "type": "object",
        ...
        "handler": _tool_template_list,
        "annotations": {"readOnlyHint": True},
    },
    "mural_mural_poll": {
        "title": "Poll mural until condition",
        "description": (
            "Poll a mural until a dotted-path condition matches or the "
            "timeout expires.\n\n"
            "`condition` is `'path op value'` where `op` is `==` or `!=` "
            "(e.g. `status==active`). Sleeps with bounded exponential backoff "
            "between polls (interval × 1/2/4, capped at 60s). Returns "
            "`{matched, attempts, condition, mural}`. Read-only."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "interval_s": {
                    "type": "number",
                    "exclusiveMinimum": 0,
                    "maximum": POLL_MAX_INTERVAL_S,
                    "default": POLL_DEFAULT_INTERVAL_S,
                },
                "timeout_s": {
                    "type": "number",
                    "exclusiveMinimum": 0,
                    "maximum": POLL_MAX_TIMEOUT_S,
                    "default": POLL_DEFAULT_TIMEOUT_S,
                },
            },
            "required": ["mural", "condition"],
            "additionalProperties": False,
        },
        "handler": _tool_mural_poll,
        "annotations": {"readOnlyHint": True},
    },
    "mural_archive": {
        "title": "Archive mural",
        "description": (
            "Archive a mural by setting its status to `archived`.\n\n"
            "Calls `PATCH /murals/{mural}` with `{status: 'archived'}`. "
            "Reversible via `mural_unarchive`."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_mural_archive,
        "destructive": True,
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_unarchive": {
        "title": "Unarchive mural",
        "description": (
            "Restore an archived mural by setting its status to `active`.\n\n"
            "Calls `PATCH /murals/{mural}` with `{status: 'active'}`."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_mural_unarchive,
        "destructive": True,
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_layout_grid": {
        "title": "Place widgets in a grid layout",
        "description": (
            "Place widgets row-major in a fixed-column grid inside an area.\n\n"
            "Returns `{computed_metadata, widgets, skipped, warnings}` where "
            "each placed widget is patched with computed `x`/`y`/`width`/"
            "`height` and tagged with `auto-layout-hash:<digest>`. Refuses "
            "with the `AREA_CAPACITY_EXCEEDED` envelope when the bounding "
            "rectangle would exceed the area's capacity."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "cell_width": {"type": "number", "exclusiveMinimum": 0},
                "cell_height": {"type": "number", "exclusiveMinimum": 0},
            ...
            "required": ["mural", "area", "widgets", "columns"],
            "additionalProperties": False,
        },
        "handler": _tool_layout_grid,
        "destructive": True,
        "creates": True,
        "required_scopes": ["murals:write"],
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_layout_cluster": {
        "title": "Place widgets in a square cluster",
        "description": (
            "Place widgets in a square-ish cluster inside an area.\n\n"
            "Columns are derived as ceil(sqrt(N)). Returns `{computed_"
            "metadata, widgets, skipped, warnings}` and refuses with the "
            "`AREA_CAPACITY_EXCEEDED` envelope on overflow."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
            ...
            "required": ["mural", "area", "widgets"],
            "additionalProperties": False,
        },
        "handler": _tool_layout_cluster,
        "destructive": True,
        "creates": True,
        "required_scopes": ["murals:write"],
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    ...
    "mural_layout_column": {
        "title": "Place widgets in a single column",
        "description": (
            "Place widgets in a single vertical column inside an area.\n\n"
            "Returns `{computed_metadata, widgets, skipped, warnings}` and "
            "refuses with the `AREA_CAPACITY_EXCEEDED` envelope on overflow."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    "mural_layout_row": {
        "title": "Place widgets in a single row",
        "description": (
            "Place widgets in a single horizontal row inside an area.\n\n"
            "Returns `{computed_metadata, widgets, skipped, warnings}` and "
            "refuses with the `AREA_CAPACITY_EXCEEDED` envelope on overflow."
        ),
        "input_schema": {
    ...
    "mural_find": {
        "title": "Search murals by title",
        "description": (
            "Search murals in a workspace by trigram-similarity on title.\n\n"
            "Returns `{candidates: [...top-N], confirmation_required: True, "
            "search_endpoint_pending: True}`. When `workspace` is omitted, "
            "defaults to `MURAL_DEFAULT_WORKSPACE`. Read-only."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_mural_find,
        "annotations": {"readOnlyHint": True},
    },
    "mural_workspace_summary": {
        "title": "Workspace summary",
        "description": (
            "Summarize a workspace.\n\n"
            "Returns `{workspace_id, rooms, murals, murals_archived, "
            "murals_active}`. When `workspace` is omitted, defaults to "
            "`MURAL_DEFAULT_WORKSPACE`. Read-only."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_workspace_summary,
        "annotations": {"readOnlyHint": True},
    },
    "mural_parking_lot_sweep": {
        "title": "Sweep parking-lot widgets",
        "description": (
            "List widgets in a parking-lot area or carrying the "
            "`parking-lot` tag.\n\n"
            "Returns `{mural_id, widgets}`. Read-only."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_parking_lot_sweep,
        "annotations": {"readOnlyHint": True},
    },
    "mural_bootstrap_dt_board": {
        "title": "Bootstrap a Design Thinking board",
        "description": (
            "Create or reuse a Design Thinking mural for a method (1..9).\n\n"
            "Idempotent by `dt-method:<n>` reserved tag. Creates the mural "
            "and per-section areas using `assets/dt-sections.default.yml` "
            "(deep-merged with optional override). Returns "
            "`{mural_id, areas, idempotent?}`."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "workspace": {"type": "string", "minLength": 1},
                "room": {"type": "string", "minLength": 1},
                "method": {"type": "integer", "minimum": 1, "maximum": 9},
                "title": {"type": "string", "minLength": 1},
                "override_path": {"type": "string", "minLength": 1},
            },
            "required": ["workspace", "room", "method"],
            "additionalProperties": False,
        },
        "handler": _tool_bootstrap_dt_board,
        "destructive": True,
        "creates": True,
        "required_scopes": ["murals:write"],
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_bootstrap_ux_board": {
        "title": "Bootstrap a UX research board",
        "description": (
            "Add the five UX research areas (JTBD, Journey Stages, Pain "
            "Points, Opportunities, Accessibility) to an existing mural.\n\n"
            "Idempotent by area title: existing areas with the same title "
            "are preserved. Tags the mural with `ux-board`. Returns "
            "`{mural_id, workspace_id, idempotent, areas: [{id, label, "
            "anchor_widget_id}, ...]}`."
        ),
        "input_schema": {
            "type": "object",
            ...
            "required": ["workspace", "mural"],
            "additionalProperties": False,
        },
        "handler": _tool_bootstrap_ux_board,
        "destructive": True,
        "creates": True,
        "required_scopes": ["murals:write"],
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_populate_dt_section": {
        "title": "Populate a Design Thinking section",
        "description": (
            "Place items into a Design Thinking section area.\n\n"
            "Ensures `dt-method:<n>` and `dt-section:<name>` tags, then "
            "delegates to `mural_layout_cluster`. Refuses with "
            "`AREA_CAPACITY_EXCEEDED` when the area cannot fit the items."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
            ...
            "required": ["mural", "area", "method", "section", "items"],
            "additionalProperties": False,
        },
        "handler": _tool_populate_dt_section,
        "destructive": True,
        "creates": True,
        "required_scopes": ["murals:write"],
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_create_affinity_cluster": {
        "title": "Create affinity clusters",
        "description": (
            "Place pre-clustered items into an area with cluster labels.\n\n"
            "Accepts `clusters: [{label, items}]` (no LLM). Each cluster "
            "advances origin by `envelope.width + 200`. Tags items with "
            "`dt-method:3`, `dt-section:affinity`, and "
            "`cluster-label:<slug>`. Refuses with `AREA_CAPACITY_EXCEEDED`."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
            ...
            "required": ["mural", "area", "clusters"],
            "additionalProperties": False,
        },
        "handler": _tool_create_affinity_cluster,
        "destructive": True,
        "creates": True,
        "required_scopes": ["murals:write"],
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_repair_tag_drift": {
        "title": "Repair widget tag drift",
        "description": (
            "Re-assert reserved tags on widgets in a mural.\n\n"
            "Re-applies missing `auto-layout-hash:`, `dt-method:`, "
            "`dt-section:`, `cluster-label:`, and `ai-author:` tags. "
            "Non-destructive: only adds missing tags."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_repair_tag_drift,
        "annotations": {"idempotentHint": True},
    },
    "mural_lineage_lookup": {
        "title": "Look up widgets by lineage marker",
        "description": (
            "Find widgets carrying a Design Thinking lineage prefix.\n\n"
            "Scans every widget in `mural_id` and parses the leading "
            "`[dt:method=N section=NAME run=ID]` marker. Filters `run_id`, "
            "`method`, and `section` are optional and combine with AND "
            "semantics. Read-only."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "mural_id": {"type": "string", "minLength": 1},
        ...
        "handler": _tool_mural_lineage_lookup,
        "annotations": {"readOnlyHint": True},
    },
    "mural_voting_session_create": {
        "title": "Create voting session",
        "description": (
            "Create a voting session on a mural.\n\n"
            "`body` is a JSON object passed straight through to "
            "`POST /murals/{mural}/voting-sessions`. The session is created "
            "in its initial state; call `mural_voting_session_open` to start "
            "voting."
        ),
        "input_schema": {
            "type": "object",
        ...
        "handler": _tool_voting_session_create,
        "destructive": True,
        "creates": True,
        "required_scopes": ["murals:write"],
        "annotations": {"destructiveHint": True},
    },
    "mural_voting_session_get": {
        "title": "Get voting session",
        "description": (
            "Fetch a single voting session record by id.\n\n"
            "Returns the raw session payload from "
            "`GET /murals/{mural}/voting-sessions/{session}`."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
            ...
            "required": ["mural", "session"],
            "additionalProperties": False,
        },
        "handler": _tool_voting_session_get,
        "annotations": {"readOnlyHint": True},
    },
    "mural_voting_session_list": {
        "title": "List voting sessions",
        "description": (
            "List voting sessions on a mural with cursor pagination.\n\n"
            "Honors `limit` (total cap) and `page_size` (per-page query)."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_voting_session_list,
        "annotations": {"readOnlyHint": True},
    },
    "mural_voting_session_open": {
        "title": "Open voting session",
        "description": (
            "Mark a voting session as `active` so participants can vote.\n\n"
            "Issues `PATCH /murals/{mural}/voting-sessions/{session}` with "
            "`{status: active}`."
        ),
        "input_schema": {
            "type": "object",
        ...
        "handler": _tool_voting_session_open,
        "destructive": True,
        "required_scopes": ["murals:write"],
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_voting_session_close": {
        "title": "Close voting session",
        "description": (
            "Mark a voting session as `closed` so no further votes are "
            "accepted.\n\n"
            "Issues `PATCH /murals/{mural}/voting-sessions/{session}` with "
            "`{status: closed}`."
        ),
        ...
        "handler": _tool_voting_session_close,
        "destructive": True,
        "required_scopes": ["murals:write"],
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_voting_session_delete": {
        "title": "Delete voting session",
        "description": (
            "Delete a voting session by id.\n\n"
            "Removes the session and any associated vote records."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_voting_session_delete,
        "destructive": True,
        "required_scopes": ["murals:write"],
        "annotations": {"destructiveHint": True, "idempotentHint": True},
    },
    "mural_voting_results": {
        "title": "Get voting results",
        "description": (
            "Fetch tallied results for a voting session.\n\n"
            "Returns the payload from "
            "`GET /murals/{mural}/voting-sessions/{session}/results`."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_voting_results,
        "annotations": {"readOnlyHint": True},
    },
    "mural_voting_poll": {
        "title": "Poll voting session",
        "description": (
            "Poll a voting session record until a dotted-path condition "
            "matches or the timeout elapses.\n\n"
            "`condition` uses the same `path==value` grammar as "
            "`mural_mural_poll` (e.g. `status==closed`). Returns "
            "`{matched, attempts, condition, session}`."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
            ...
            "required": ["mural", "session", "condition"],
            "additionalProperties": False,
        },
        "handler": _tool_voting_poll,
        "annotations": {"readOnlyHint": True},
    },
    "mural_voting_run": {
        "title": "Run end-to-end voting session",
        "description": (
            "Composite: create→open→poll→close→results in one call.\n\n"
            "Requires confirmation: the first call returns "
            "`{confirmation_required, confirmation_id, preview}`; replay the "
            "call with `confirmation_id` to execute. Returns "
            "`{session, results, poll, closed_on_timeout, warnings}`. "
            "Defaults: `poll_condition=status==closed`, "
            f"`poll_interval={POLL_DEFAULT_INTERVAL_S}`, "
            f"`poll_timeout={POLL_DEFAULT_TIMEOUT_S}`, "
        ...
        "input_schema": {
            ...
            "properties": {
                "mural": {"type": "string", "minLength": 1},
                "body": {"type": "object"},
                "poll_condition": {"type": "string", "minLength": 1},
                "poll_interval": {"type": "number", "exclusiveMinimum": 0},
                "poll_timeout": {"type": "number", "exclusiveMinimum": 0},
                "close_on_timeout": {"type": "boolean"},
                "confirmation_id": {"type": "string", "minLength": 1},
            },
            "required": ["mural", "body"],
            "additionalProperties": False,
        },
        "handler": _tool_voting_run,
        "destructive": True,
        "creates": True,
        "required_scopes": ["murals:write"],
        "annotations": {"destructiveHint": True},
    },
    "mural_workspace_search": {
        "title": "Search murals in a workspace",
        "description": (
            "Full-text search for murals in a workspace via "
            "`GET /search/{workspace}/murals`.\n\n"
            "`query` maps to the `q` query parameter. Cursor paginated; "
            "honors `limit` (total cap) and `page_size` (per-page)."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    "mural_auth_status": {
        "title": "Auth status",
        "description": (
            "Report whether the local token store is authenticated.\n\n"
            "Returns `{authenticated, token_store, credential_file, "
            "credential_file_exists}` plus, when authenticated, "
            "`{profile, scope, expires_at, has_refresh_token}`. "
            "Reads the encrypted token store and resolves the credential "
            "file path for the active profile. Does not perform a network "
            "call. Read-only."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "profile": {
                    ...
                    "description": (
                        ...
                        "`MURAL_PROFILE`, the token store's active profile, "
                        "and finally the default profile."
                    ),
                },
            },
            "additionalProperties": False,
    ...
    "mural_spatial_widgets_in_shape": {
        "title": "Filter widgets contained by a shape",
        "description": (
            "Filter widgets contained by a Mural shape (frame, area, or "
            "arbitrary widget) using AABB or center-point inclusion.\n\n"
            "Fetches the shape via `GET /murals/{mural_id}/widgets/{shape_id}`, "
            "drains every widget on the mural, and returns those whose center "
            "(`mode=center`, default) or full bounding box (`mode=bbox`) lies "
            "inside the shape's AABB. `rotation_aware=true` (or the env flag "
            "`MURAL_SPATIAL_ROTATION_ENABLED=1`) expands the shape AABB to "
            "cover its rotated extent. Read-only."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "mode": {"type": "string", "enum": ["center", "bbox"]},
                "rotation_aware": {"type": "boolean"},
            },
            "required": ["mural_id", "shape_id"],
            "additionalProperties": False,
        },
        "handler": _tool_spatial_widgets_in_shape,
        "annotations": {"readOnlyHint": True},
    },
    "mural_spatial_widgets_in_region": {
        "title": "Filter widgets inside a rectangle",
        "description": (
            "Filter widgets inside an axis-aligned rectangle using AABB or "
            "center-point inclusion.\n\n"
            "`(x, y, w, h)` define the region; negative `w` or `h` are "
            "sign-corrected via `safe_rect` so callers can pass either corner "
            "in any order. `mode=center` (default) keeps widgets whose center "
            "lies inside the region; `mode=bbox` requires the full widget "
            "bounding box to be contained. Read-only."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
            ...
            "required": ["mural_id", "x", "y", "w", "h"],
            "additionalProperties": False,
        },
        "handler": _tool_spatial_widgets_in_region,
        "annotations": {"readOnlyHint": True},
    },
    "mural_spatial_pairwise_overlaps": {
        "title": "Find overlapping widget pairs",
        "description": (
            "Find overlapping widget id pairs across the mural canvas using "
            "a shapely STR R-tree index.\n\n"
            "Drains every widget on the mural, builds an STRtree from each "
            "widget's AABB, and returns the deterministic list of "
            "`{a, b}` id pairs with `a < b`. `predicate=intersects` "
            "(default) emits any pair whose AABBs touch or overlap; "
            "`predicate=contains` keeps only pairs where one AABB fully "
            "contains the other. `rotation_aware=true` (or the env flag "
            "`MURAL_SPATIAL_ROTATION_ENABLED=1`) expands AABBs to cover "
            "rotated extents. Read-only."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
        ...
        "handler": _tool_spatial_pairwise_overlaps,
        "annotations": {"readOnlyHint": True},
    },
    "mural_spatial_cluster": {
        "title": "Cluster widgets by spatial proximity",
        "description": (
            "Group widgets into spatial-proximity clusters using DBSCAN "
            "over AABB centers.\n\n"
            "Drains every widget on the mural, projects each AABB center "
            "to a 2D point, and runs DBSCAN with a `scipy.spatial.cKDTree` "
            "for `eps_px`-radius neighborhood queries. `eps_px` (default "
            "120) sets the neighborhood radius; `min_samples` (default 2) "
            "sets the density threshold below which points are treated as "
            "noise. Returns clusters as `{members: [widget_id, ...]}` "
            "records sorted by descending size with a stable lexicographic "
            "tiebreak on the smallest member id. Read-only."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "mural_id": {"type": "string", "minLength": 1},
                "eps_px": {"type": "number", "exclusiveMinimum": 0},
                "min_samples": {"type": "integer", "minimum": 1},
            },
            "required": ["mural_id"],
            "additionalProperties": False,
        },
        "handler": _tool_spatial_cluster,
        "annotations": {"readOnlyHint": True},
    },
    "mural_spatial_sort_along_axis": {
        "title": "Sort widgets by AABB-center projection along an axis",
        "description": (
            "Order widgets by AABB-center projection along x, y, or the "
            "(1,1)/sqrt(2) diagonal axis.\n\n"
            "Drains every widget on the mural, computes each AABB center "
            "via the rotation-aware shape adapter, and projects centers "
            "onto the axis vector selected by `axis` (one of `x`, `y`, "
            "`diagonal`; default `x`). When both `origin_x` and "
            "`origin_y` are supplied the sort key becomes the signed "
            "projection of `(center - origin)` along the axis so callers "
            "can order widgets by distance from an anchor; supplying only "
            "one origin coordinate is rejected. Ties break by widget id "
            "for deterministic output. Returns the widget objects in "
        ...
        "handler": _tool_spatial_sort_along_axis,
        "annotations": {"readOnlyHint": True},
    },
    "mural_spatial_arrow_graph": {
        "title": "Build an arrow-anchored directed multigraph for a mural",
        "description": (
            "Snap arrow widgets to nearest non-arrow widget centers and "
            "return the resulting directed multigraph.\n\n"
            "Drains every widget on the mural, partitions arrow widgets "
            "from the rest, and snaps each arrow's start and end points "
            "to the nearest non-arrow widget AABB center within "
            "`snap_radius` (Euclidean pixels; default 24). Arrows missing "
            "either anchor are dropped. Returns the summary by default "
            "with `nodes`, `edges` (`{id, source, target}`), and `stats` "
            "(`node_count`, `edge_count`, `is_dag`). `format` `full` "
            "augments each edge with its `arrow_widget`; `format` `dot` "
            "returns `{format: 'dot', text: ...}` containing a Graphviz "
            "`digraph` document. Read-only."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "mural_id": {"type": "string", "minLength": 1},
                "snap_radius": {"type": "number", "exclusiveMinimum": 0},
                "format": {
                    "type": "string",
        ...
        "handler": _tool_spatial_arrow_graph,
        "annotations": {"readOnlyHint": True},
    },
}
...
def _run_mcp_stdio(
    ...
    Tool calls execute on a single-worker ``ThreadPoolExecutor`` so requests
    are processed in strict FIFO order, one at a time. A per-call wall-clock
    timeout of ``MURAL_TOOL_TIMEOUT_SECS`` seconds bounds any tool that
    blocks on the network or a slow remote API. On timeout the dispatcher
    emits JSON-RPC error ``-32000`` with ``data.tool`` and
    ``data.timeout_secs`` so the host can recover, and the server continues
    serving subsequent frames. Inbound frames are size-capped via
    ``_read_frame`` (``MURAL_MAX_FRAME_BYTES``); outbound HTTP responses are
    size-capped via ``_read_capped`` (``MURAL_MAX_BODY_BYTES``).
    """
    if stdin is None:
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    bootstrap = auth_sub.add_parser(
        ...
        description=(
            ...
            "file|env-only}). Subsequent CLI / MCP runs resolve credentials "
    ...
    mcp = sub.add_parser("mcp", help="Run the embedded MCP stdio server")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(_redact(f"internal error: {exc}"), file=sys.stderr)
